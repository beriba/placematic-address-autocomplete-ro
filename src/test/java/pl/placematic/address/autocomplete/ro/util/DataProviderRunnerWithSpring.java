...
public class DataProviderRunnerWithSpring extends SpringJUnit4ClassRunner {
     ...
     */
    public DataProviderRunnerWithSpring(Class<?> clazz) throws InitializationError {
        super(clazz);
    }
        ...
        for (FrameworkMethod testMethod : getTestClassInt().getAnnotatedMethods(UseDataProvider.class)) {
            ...
            if (dataProviderMethod == null) {
                errors.add(new Exception(String.format(
                        "No valid dataprovider found for test %s. By convention the dataprovider method fieldName must either be equal to the test methods fieldName, have a prefix of 'dataProvider' instead of 'test' or is overridden by using @UseDataProvider#value().",
                        testMethod.getName())));
